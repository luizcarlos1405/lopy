...
<div class="relative text-2xl overflow-x-visible text-center">
  <div
  ...
  {#if open}
    <div
      class="absolute flex flex-col bottom-14 -left-9 h-80 w-max rounded shadow-lg overflow-hidden"
      use:clickOutside
      on:clickedOutside="{close}"
    >
      <div
        class="z-30 grid grid-cols-9 gap-1 p-2 w-full h-full bg-white overflow-y-scroll overscroll-contain"
      >
      ...
      <div
        class="sticky bottom-0 z-50 p-2 bg-white rounded-b shadow grid grid-cols-9 gap-1 justify-items-center"
      >
        {#each Object.entries(iconByEmojiGroup) as [group, icon]}
          <span
            class="font-xs cursor-pointer fill-current bg-white text-gray-500"
            on:click="{() => {
              currentGroup = group;
            }}"
            class:text-secondary="{group === currentGroup}"
            class:text-gray-500="{group !== currentGroup}"
          >
            {@html icon}
          </span>
        {/each}
