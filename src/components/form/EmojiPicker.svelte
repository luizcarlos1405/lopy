<script>
  import * as dataByEmoji from 'unicode-emoji-json/data-by-emoji';
  ...
  // The eye in a speech bubble emoji does not work in the mobile chrome browser
  const emojisData = Object.entries(dataByEmoji.default)
    .filter(([emoji, { emoji_version }]) => +emoji_version <= 11 && emoji !== "👁️‍🗨️")
    .map(([emoji, { group }]) => ({ emoji, group }));

  let currentGroup = 'Smileys & Emotion';

  const handleClickedOutsie = () => {
    open = false;
  };
...
>
  ...
  >
    <div
      class="z-30 grid h-full w-full grid-cols-9 gap-1 overflow-y-scroll overscroll-contain p-2"
    >
      {#each emojisData as { emoji, group }}
        {#if currentGroup === group}
          <div
            class="text-md max-h-8 max-w-lg cursor-pointer"
            on:click={() => {
              console.log(emoji);
              value = emoji;
            }}
    ...
    >
      {#each Object.entries(iconByEmojiGroup) as [group, icon]}
        <button
          class="font-xs btn btn-square btn-sm cursor-pointer fill-current p-1"
          on:click={() => {
            currentGroup = group;
          }}
          class:btn-accent={group === currentGroup}
          class:btn-ghost={group !== currentGroup}
        >
          {@html icon}
        </button>
      {/each}
